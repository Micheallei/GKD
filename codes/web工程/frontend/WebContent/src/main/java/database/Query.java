...
/*
    ...
    public DeviceItem[] queryOnlineDevice(){
        ...
        try{
            stmt = conn.createStatement();
            String sql;
            sql = String.format("SELECT * FROM DFS.DEVICE WHERE IS_ONLINE=true ORDER BY RS DESC");
            rs = stmt.executeQuery(sql);
    ...
    public int queryRequestNumbers(int fileId, int type){
        ...
        try{
            ...
            sql = String.format("SELECT COUNT(*) FROM DFS.REQUEST WHERE FRAGMENTID>='%d' "
                    + "AND FRAGMENTID<'%d' AND TYPE_='%d'", fileId*100, (fileId+1)*100, type);
            rs = stmt.executeQuery(sql);
        ...
        try{
            ...
            if (file.isFolder())
                sql = String.format("UPDATE DFS.FILE SET NAME='%s',PATH='%s',ATTRIBUTE='%s',"
                                + "TIME='%s',NOA=%d,IS_FOLDER=true WHERE id=%d;",file.getFileName(),file.getPath(),
                        file.getAttribute(),file.getTime(),file.getNoa(),file.getId());
            else
                sql = String.format("UPDATE DFS.FILE SET NAME='%s',PATH='%s',ATTRIBUTE='%s',"
                                + "TIME='%s',NOA=%d,IS_FOLDER=false WHERE id=%d;",file.getFileName(),file.getPath(),
                        file.getAttribute(),file.getTime(),file.getNoa(),file.getId());
            suc = stmt.executeUpdate(sql);
            if (suc>0)
                return 1;
        ...
        try{
            ...
            if (device.isOnline())
                sql = "UPDATE DFS.DEVICE SET IP='"+device.getIp()+"',PORT='"+device.getPort()+
                        "',IS_ONLINE=true,RS='"+device.getRs()+"',LEFTRS='"+device.getLeftrs()+
                        "'WHERE id='"+device.getId()+"';";
            else
                sql = "UPDATE DFS.DEVICE SET IP='"+device.getIp()+"',PORT='"+device.getPort()+
                        "',IS_ONLINE=false,RS='"+device.getRs()+"',LEFTRS='"+device.getLeftrs()+
                        "'WHERE id='"+device.getId()+"';";
            /*
            if (device.isOnline())
                sql = String.format("UPDATE DFS.DEVICE SET IP='%s',PORT=%d,IS_ONLINE=true"
                                + "RS=%d WHERE id=%d;",device.getIp(),device.getPort(),device.getRs(),
                        device.getId());
            else
                sql = String.format("UPDATE DFS.DEVICE SET IP='%s',PORT=%d,IS_ONLINE=false"
                                + "RS=%d WHERE id=%d;",device.getIp(),device.getPort(),device.getRs(),
                        device.getId());*/
            suc = stmt.executeUpdate(sql);
            if (suc>0)
    ...
    public int addRequest(RequestItem request){
        ...
        try{
            stmt = conn.createStatement();
            String sql;
            sql = String.format("INSERT INTO DFS.REQUEST (TYPE_,FRAGMENTID,DEVICEID) "
                            + "VALUES ('%d','%d','%d');",
